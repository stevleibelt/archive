...
use Net\Bazzline\Framework\DataMapper\DataMapperAbstract;
use Net\Bazzline\Framework\Model\ModelInterface;
use Model\Event;
use Model\EventCollection;

/**
 * @author stev leibelt
 * @since 2013-02-24
 */
class EventDataMapper extends DataMapperAbstract
{
    /**
     * @author stev leibelt
     * @since 2013-02-24
    ...
    public function find(ModelInterface $model)
    {
        ...
        foreach($entries as $entry) {
            $collection->push($this->buildEvent($entry));
        }
    ...
    {
        $statement = 'SELECT * FROM `' . $this->getTableName() . '` as `e`';

        $where = '';

        if (!is_null($event->getColor())) {
            $where .= '`e`.`color` = ' . $event->getColor();
        }
        if (!is_null($event->getDescription())) {
            $where .= '`e`.`description` like \'' . $event->getDescription() . '\'';
        }
        if (!is_null($event->getEndTime())) {
            $where .= '`e`.`end_time` = ' . $event->getEndTime();
        }
        if (!is_null($event->getGroupId())) {
            $where .= '`e`.`group_id` = ' . $event->getGroupId();
        }
        if (!is_null($event->getId())) {
            $where .= '`e`.`id` = \'' . $event->getId() . '\'';
        }
        if (!is_null($event->getIsAllDayEvent())) {
            $where .= '`e`.`is_all_day_event` = ' . $event->getIsAllDayEvent();
        }
        if (!is_null($event->getLocation())) {
            $where .= '`e`.`location` = \'' . $event->getLocation() . '\'';
        }
        if (!is_null($event->getStartTime())) {
            $where .= '`e`.`start_time` = ' . $event->getStartTime();
        }
        if (!is_null($event->getSubject())) {
            $where .= '`e`.`subject` like \'' . $event->getSubject() . '\'';
        }
        if (!is_null($event->getUserId())) {
            $where .= '`e`.`user_id` = ' . $event->getUserId();
        }

        $statement .= (strlen($where) > 0) ? ' WHERE ' . $where : '';
